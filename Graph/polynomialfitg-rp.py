...
y = 1.6
psc_sample = psc_sample[psc_sample['g-rp'] < y]
catalog_sample = catalog_sample[catalog_sample['g-rp'] < y]
...
degree = 7
cheb_fit = cheb.Chebyshev.fit(g_rp_psc, mag_psc, degree)

x_fit = np.linspace(min(g_rp_psc), max(g_rp_psc), 500)
y_fit = cheb_fit(x_fit) + 0.7

shifted_fit_psc_values = cheb_fit(psc_sample['g-rp'].values) + 0.71
shifted_fit_cat_values = cheb_fit(catalog_sample['g-rp'].values) + 0.71
...
combined1.to_csv("catalog_and_100psc_low_grp.csv", index=False)

combined2 = pd.concat([psc_high, cat_high], ignore_index=True)
combined2.to_csv("catalog_and_100psc_high_grp.csv", index=False)
